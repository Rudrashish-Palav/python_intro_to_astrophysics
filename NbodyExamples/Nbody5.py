...
dt = 0.0025* torbit#we set the timestep, dt, to 2.5 percent of an orbital time
tmax = 5.0*torbit#We only run simulation for 1 orbit.

t = 0.0# we start at t=0.0


#With the Leapfrog integration method we calculate v at the midpoint inbetween two timesteps. This integration improves accuracy. Leapfrog is accurate to 2nd order (instead of 1st order)
r = numpy.sqrt(x**2+y**2+z**2)
F = - G * Msun * Mearth / r**2
ax = F/Mearth * x/r
ay = F/Mearth * y/r
